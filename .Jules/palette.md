...
## 2026-05-04 - [Inputs generated via Obsidian Settings API lacks accessibility tags]
**Learning:** Inputs generated using the Obsidian Settings API via methods like `addText`, `addTextArea`, and `addDropdown` don't automatically set `aria-label` or `title` properties. This leaves them completely invisible to screen readers, and devoid of basic tooltip descriptions for mouse users.
**Action:** When adding or modifying these input components, it is critical to explicitly set `.setAttribute('aria-label', 'Description')` and `.setAttribute('title', 'Description')` directly onto the `inputEl` or `selectEl` to maintain A11y and UX consistency.
