...
## 2026-04-30 - Native tooltips for interactive cards and toggles
**Learning:** Even if an element has text content (like a 'Timeline' toggle button) or a role with aria-labels (like a 'status' card), adding a matching `title` attribute is crucial for providing native hover tooltips to mouse users, improving accessibility and discoverability.
**Action:** When adding or auditing interactive elements (like view toggles) or informative cards (like stat cards) with `aria-label`, ensure they also have a matching `title` attribute.
