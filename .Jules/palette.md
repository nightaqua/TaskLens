...
## 2026-05-16 - [Unified Hover Targets for UI Cards]
**Learning:** Adding an `aria-label` alone doesn't provide a native hover tooltip for mouse users on complex UI components like info cards or histograms.
**Action:** When creating visual data components (like stat cards or bar charts) that group details, ensure both `aria-label` and `title` are assigned to their container (with `role="status"` or `role="group"`) to unify the screen reader announcement and the hover tooltip.
