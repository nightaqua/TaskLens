...
export class StatsComponent {
    ...
    private renderUrgentTopic(topic: { name: string, ratio: number, urgent: number, total: number }): void {
        ...
        const details = `${String(topic.urgent)} of ${String(topic.total)} open tasks are urgent`;
        const label = `Most urgent topic: ${topic.name}. ${String(percent)}% urgent. ${details}`;
        ...
        card.createDiv('urgent-topic-details').setText(details);
    }
}