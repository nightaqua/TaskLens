...
export class StatsComponent {
    ...
    private renderPacingAnalysis(velocity: number[]): void {
        ...
        velocity.forEach((val, i) => {
            ...
            barWrapper.setAttribute('aria-label', `${days[i]}: ${String(val)} tasks`);
            barWrapper.setAttribute('title', `${days[i]}: ${String(val)} tasks`);
    ...
    private renderUrgentTopic(topic: { name: string, ratio: number, urgent: number, total: number }): void {
        ...
        card.setAttribute('role', 'status');
        const accessibleLabel = `${topic.name}: ${String(percent)}% urgent. ${String(topic.urgent)} of ${String(topic.total)} open tasks are urgent.`;
        card.setAttribute('aria-label', accessibleLabel);
        card.setAttribute('title', accessibleLabel);
